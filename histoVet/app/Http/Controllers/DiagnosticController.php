...
class DiagnosticController extends Controller
{
    public function index(){ //listar todos los elementos almacenados en esa tabla
        return response()->json(Diagnostic::all(),200); //Permission es el modelo. 200: es el OK
    }

    public function show($id){
        $the_permission = Diagnostic::find($id);
    ...
    public function store(Request $request){
        $the_permission=Diagnostic::create($request->all());
